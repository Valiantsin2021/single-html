...
const combineAllure = (folder) => {
  ...
  if (!/sinon-9.2.4.js/.test(indexHtml)) {
    ...
    indexHtml = indexHtml.replace('<script src="app.js"></script>', '<script src="sinon-9.2.4.js"></script><script src="server.js"></script><script src="app.js"></script>')
  ...
  fs.writeFileSync(`./report/report.html`, soup.toString())

  console.log(`> Saving result as ./report/report.html`)

  const { size: completeFileSize } = fs.statSync(`./report/report.html`)
